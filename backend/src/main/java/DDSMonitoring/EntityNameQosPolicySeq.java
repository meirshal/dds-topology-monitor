...
/**
* A sequence of EntityNameQosPolicy instances.
*/
public final class EntityNameQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public EntityNameQosPolicySeq() {
        super(EntityNameQosPolicy.class);
    }

    public EntityNameQosPolicySeq (int initialMaximum) {
        super(EntityNameQosPolicy.class, initialMaximum);
    }

    public EntityNameQosPolicySeq (Collection elements) {
        super(EntityNameQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, EntityNameQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' EntityNameQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(EntityNameQosPolicy.create());
