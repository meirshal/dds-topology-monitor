...
/**
* A sequence of BuiltinTopicReaderResourceLimits_t instances.
*/
public final class BuiltinTopicReaderResourceLimits_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BuiltinTopicReaderResourceLimits_tSeq() {
        super(BuiltinTopicReaderResourceLimits_t.class);
    }

    public BuiltinTopicReaderResourceLimits_tSeq (int initialMaximum) {
        super(BuiltinTopicReaderResourceLimits_t.class, initialMaximum);
    }

    public BuiltinTopicReaderResourceLimits_tSeq (Collection elements) {
        super(BuiltinTopicReaderResourceLimits_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BuiltinTopicReaderResourceLimits_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BuiltinTopicReaderResourceLimits_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BuiltinTopicReaderResourceLimits_t.create());
