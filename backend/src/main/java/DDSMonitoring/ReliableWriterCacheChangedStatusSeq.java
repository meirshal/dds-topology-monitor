...
/**
* A sequence of ReliableWriterCacheChangedStatus instances.
*/
public final class ReliableWriterCacheChangedStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ReliableWriterCacheChangedStatusSeq() {
        super(ReliableWriterCacheChangedStatus.class);
    }

    public ReliableWriterCacheChangedStatusSeq (int initialMaximum) {
        super(ReliableWriterCacheChangedStatus.class, initialMaximum);
    }

    public ReliableWriterCacheChangedStatusSeq (Collection elements) {
        super(ReliableWriterCacheChangedStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ReliableWriterCacheChangedStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ReliableWriterCacheChangedStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ReliableWriterCacheChangedStatus.create());
