...
/**
* A sequence of DataWriterProtocolQosPolicy instances.
*/
public final class DataWriterProtocolQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataWriterProtocolQosPolicySeq() {
        super(DataWriterProtocolQosPolicy.class);
    }

    public DataWriterProtocolQosPolicySeq (int initialMaximum) {
        super(DataWriterProtocolQosPolicy.class, initialMaximum);
    }

    public DataWriterProtocolQosPolicySeq (Collection elements) {
        super(DataWriterProtocolQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataWriterProtocolQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataWriterProtocolQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataWriterProtocolQosPolicy.create());
