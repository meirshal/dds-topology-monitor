...
/**
* A sequence of SystemResourceLimitsQosPolicy instances.
*/
public final class SystemResourceLimitsQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SystemResourceLimitsQosPolicySeq() {
        super(SystemResourceLimitsQosPolicy.class);
    }

    public SystemResourceLimitsQosPolicySeq (int initialMaximum) {
        super(SystemResourceLimitsQosPolicy.class, initialMaximum);
    }

    public SystemResourceLimitsQosPolicySeq (Collection elements) {
        super(SystemResourceLimitsQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SystemResourceLimitsQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SystemResourceLimitsQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SystemResourceLimitsQosPolicy.create());
