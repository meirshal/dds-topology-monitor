...
/**
* A sequence of PresentationQosPolicy instances.
*/
public final class PresentationQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PresentationQosPolicySeq() {
        super(PresentationQosPolicy.class);
    }

    public PresentationQosPolicySeq (int initialMaximum) {
        super(PresentationQosPolicy.class, initialMaximum);
    }

    public PresentationQosPolicySeq (Collection elements) {
        super(PresentationQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PresentationQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PresentationQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PresentationQosPolicy.create());
