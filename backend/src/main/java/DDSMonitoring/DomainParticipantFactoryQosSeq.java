...
/**
* A sequence of DomainParticipantFactoryQos instances.
*/
public final class DomainParticipantFactoryQosSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DomainParticipantFactoryQosSeq() {
        super(DomainParticipantFactoryQos.class);
    }

    public DomainParticipantFactoryQosSeq (int initialMaximum) {
        super(DomainParticipantFactoryQos.class, initialMaximum);
    }

    public DomainParticipantFactoryQosSeq (Collection elements) {
        super(DomainParticipantFactoryQos.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DomainParticipantFactoryQos.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DomainParticipantFactoryQos objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DomainParticipantFactoryQos.create());
