...
/**
* A sequence of DataWriterCacheStatus instances.
*/
public final class DataWriterCacheStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataWriterCacheStatusSeq() {
        super(DataWriterCacheStatus.class);
    }

    public DataWriterCacheStatusSeq (int initialMaximum) {
        super(DataWriterCacheStatus.class, initialMaximum);
    }

    public DataWriterCacheStatusSeq (Collection elements) {
        super(DataWriterCacheStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataWriterCacheStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataWriterCacheStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataWriterCacheStatus.create());
