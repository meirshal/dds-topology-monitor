...
/**
* A sequence of DestinationOrderQosPolicyKind instances.
*/
public final class DestinationOrderQosPolicyKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DestinationOrderQosPolicyKindSeq() {
        super(DestinationOrderQosPolicyKind.class);
    }

    public DestinationOrderQosPolicyKindSeq (int initialMaximum) {
        super(DestinationOrderQosPolicyKind.class, initialMaximum);
    }

    public DestinationOrderQosPolicyKindSeq (Collection elements) {
        super(DestinationOrderQosPolicyKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DestinationOrderQosPolicyKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DestinationOrderQosPolicyKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DestinationOrderQosPolicyKind.create());
