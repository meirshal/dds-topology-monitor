...
/**
* A sequence of SubscriptionMatchedStatus instances.
*/
public final class SubscriptionMatchedStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SubscriptionMatchedStatusSeq() {
        super(SubscriptionMatchedStatus.class);
    }

    public SubscriptionMatchedStatusSeq (int initialMaximum) {
        super(SubscriptionMatchedStatus.class, initialMaximum);
    }

    public SubscriptionMatchedStatusSeq (Collection elements) {
        super(SubscriptionMatchedStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SubscriptionMatchedStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SubscriptionMatchedStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SubscriptionMatchedStatus.create());
