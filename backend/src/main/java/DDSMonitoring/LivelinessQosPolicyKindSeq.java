...
/**
* A sequence of LivelinessQosPolicyKind instances.
*/
public final class LivelinessQosPolicyKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LivelinessQosPolicyKindSeq() {
        super(LivelinessQosPolicyKind.class);
    }

    public LivelinessQosPolicyKindSeq (int initialMaximum) {
        super(LivelinessQosPolicyKind.class, initialMaximum);
    }

    public LivelinessQosPolicyKindSeq (Collection elements) {
        super(LivelinessQosPolicyKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LivelinessQosPolicyKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LivelinessQosPolicyKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LivelinessQosPolicyKind.create());
