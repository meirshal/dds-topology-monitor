...
/**
* A sequence of DatabaseQosPolicy instances.
*/
public final class DatabaseQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DatabaseQosPolicySeq() {
        super(DatabaseQosPolicy.class);
    }

    public DatabaseQosPolicySeq (int initialMaximum) {
        super(DatabaseQosPolicy.class, initialMaximum);
    }

    public DatabaseQosPolicySeq (Collection elements) {
        super(DatabaseQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DatabaseQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DatabaseQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DatabaseQosPolicy.create());
