...
/**
* A sequence of DataWriterProtocolStatus instances.
*/
public final class DataWriterProtocolStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataWriterProtocolStatusSeq() {
        super(DataWriterProtocolStatus.class);
    }

    public DataWriterProtocolStatusSeq (int initialMaximum) {
        super(DataWriterProtocolStatus.class, initialMaximum);
    }

    public DataWriterProtocolStatusSeq (Collection elements) {
        super(DataWriterProtocolStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataWriterProtocolStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataWriterProtocolStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataWriterProtocolStatus.create());
