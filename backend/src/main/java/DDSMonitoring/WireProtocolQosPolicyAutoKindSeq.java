...
/**
* A sequence of WireProtocolQosPolicyAutoKind instances.
*/
public final class WireProtocolQosPolicyAutoKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public WireProtocolQosPolicyAutoKindSeq() {
        super(WireProtocolQosPolicyAutoKind.class);
    }

    public WireProtocolQosPolicyAutoKindSeq (int initialMaximum) {
        super(WireProtocolQosPolicyAutoKind.class, initialMaximum);
    }

    public WireProtocolQosPolicyAutoKindSeq (Collection elements) {
        super(WireProtocolQosPolicyAutoKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, WireProtocolQosPolicyAutoKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' WireProtocolQosPolicyAutoKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(WireProtocolQosPolicyAutoKind.create());
