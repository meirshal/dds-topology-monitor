...
/**
* A sequence of ReceiverPoolQosPolicy instances.
*/
public final class ReceiverPoolQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ReceiverPoolQosPolicySeq() {
        super(ReceiverPoolQosPolicy.class);
    }

    public ReceiverPoolQosPolicySeq (int initialMaximum) {
        super(ReceiverPoolQosPolicy.class, initialMaximum);
    }

    public ReceiverPoolQosPolicySeq (Collection elements) {
        super(ReceiverPoolQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ReceiverPoolQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ReceiverPoolQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ReceiverPoolQosPolicy.create());
