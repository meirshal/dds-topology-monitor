...
/**
* A sequence of TransportEncapsulationQosPolicy instances.
*/
public final class TransportEncapsulationQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportEncapsulationQosPolicySeq() {
        super(TransportEncapsulationQosPolicy.class);
    }

    public TransportEncapsulationQosPolicySeq (int initialMaximum) {
        super(TransportEncapsulationQosPolicy.class, initialMaximum);
    }

    public TransportEncapsulationQosPolicySeq (Collection elements) {
        super(TransportEncapsulationQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportEncapsulationQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportEncapsulationQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportEncapsulationQosPolicy.create());
