...
/**
* A sequence of PresentationQosPolicyAccessScopeKind instances.
*/
public final class PresentationQosPolicyAccessScopeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PresentationQosPolicyAccessScopeKindSeq() {
        super(PresentationQosPolicyAccessScopeKind.class);
    }

    public PresentationQosPolicyAccessScopeKindSeq (int initialMaximum) {
        super(PresentationQosPolicyAccessScopeKind.class, initialMaximum);
    }

    public PresentationQosPolicyAccessScopeKindSeq (Collection elements) {
        super(PresentationQosPolicyAccessScopeKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PresentationQosPolicyAccessScopeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PresentationQosPolicyAccessScopeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PresentationQosPolicyAccessScopeKind.create());
