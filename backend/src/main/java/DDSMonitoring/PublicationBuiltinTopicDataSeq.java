...
/**
* A sequence of PublicationBuiltinTopicData instances.
*/
public final class PublicationBuiltinTopicDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PublicationBuiltinTopicDataSeq() {
        super(PublicationBuiltinTopicData.class);
    }

    public PublicationBuiltinTopicDataSeq (int initialMaximum) {
        super(PublicationBuiltinTopicData.class, initialMaximum);
    }

    public PublicationBuiltinTopicDataSeq (Collection elements) {
        super(PublicationBuiltinTopicData.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PublicationBuiltinTopicData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PublicationBuiltinTopicData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PublicationBuiltinTopicData.create());
