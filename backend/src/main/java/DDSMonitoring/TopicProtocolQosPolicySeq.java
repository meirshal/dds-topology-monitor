...
/**
* A sequence of TopicProtocolQosPolicy instances.
*/
public final class TopicProtocolQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TopicProtocolQosPolicySeq() {
        super(TopicProtocolQosPolicy.class);
    }

    public TopicProtocolQosPolicySeq (int initialMaximum) {
        super(TopicProtocolQosPolicy.class, initialMaximum);
    }

    public TopicProtocolQosPolicySeq (Collection elements) {
        super(TopicProtocolQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TopicProtocolQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TopicProtocolQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TopicProtocolQosPolicy.create());
