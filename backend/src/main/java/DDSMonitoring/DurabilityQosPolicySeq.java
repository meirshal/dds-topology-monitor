...
/**
* A sequence of DurabilityQosPolicy instances.
*/
public final class DurabilityQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DurabilityQosPolicySeq() {
        super(DurabilityQosPolicy.class);
    }

    public DurabilityQosPolicySeq (int initialMaximum) {
        super(DurabilityQosPolicy.class, initialMaximum);
    }

    public DurabilityQosPolicySeq (Collection elements) {
        super(DurabilityQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DurabilityQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DurabilityQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DurabilityQosPolicy.create());
