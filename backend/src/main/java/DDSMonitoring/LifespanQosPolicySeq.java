...
/**
* A sequence of LifespanQosPolicy instances.
*/
public final class LifespanQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LifespanQosPolicySeq() {
        super(LifespanQosPolicy.class);
    }

    public LifespanQosPolicySeq (int initialMaximum) {
        super(LifespanQosPolicy.class, initialMaximum);
    }

    public LifespanQosPolicySeq (Collection elements) {
        super(LifespanQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LifespanQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LifespanQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LifespanQosPolicy.create());
