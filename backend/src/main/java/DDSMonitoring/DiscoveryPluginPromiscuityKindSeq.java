...
/**
* A sequence of DiscoveryPluginPromiscuityKind instances.
*/
public final class DiscoveryPluginPromiscuityKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DiscoveryPluginPromiscuityKindSeq() {
        super(DiscoveryPluginPromiscuityKind.class);
    }

    public DiscoveryPluginPromiscuityKindSeq (int initialMaximum) {
        super(DiscoveryPluginPromiscuityKind.class, initialMaximum);
    }

    public DiscoveryPluginPromiscuityKindSeq (Collection elements) {
        super(DiscoveryPluginPromiscuityKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DiscoveryPluginPromiscuityKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DiscoveryPluginPromiscuityKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DiscoveryPluginPromiscuityKind.create());
