...
/**
* A sequence of ReliabilityQosPolicy instances.
*/
public final class ReliabilityQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ReliabilityQosPolicySeq() {
        super(ReliabilityQosPolicy.class);
    }

    public ReliabilityQosPolicySeq (int initialMaximum) {
        super(ReliabilityQosPolicy.class, initialMaximum);
    }

    public ReliabilityQosPolicySeq (Collection elements) {
        super(ReliabilityQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ReliabilityQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ReliabilityQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ReliabilityQosPolicy.create());
