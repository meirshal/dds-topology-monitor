...
/**
* A sequence of DataWriterResourceLimitsInstanceReplacementKind instances.
*/
public final class DataWriterResourceLimitsInstanceReplacementKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataWriterResourceLimitsInstanceReplacementKindSeq() {
        super(DataWriterResourceLimitsInstanceReplacementKind.class);
    }

    public DataWriterResourceLimitsInstanceReplacementKindSeq (int initialMaximum) {
        super(DataWriterResourceLimitsInstanceReplacementKind.class, initialMaximum);
    }

    public DataWriterResourceLimitsInstanceReplacementKindSeq (Collection elements) {
        super(DataWriterResourceLimitsInstanceReplacementKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataWriterResourceLimitsInstanceReplacementKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataWriterResourceLimitsInstanceReplacementKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataWriterResourceLimitsInstanceReplacementKind.create());
