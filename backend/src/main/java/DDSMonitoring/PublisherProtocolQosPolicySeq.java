...
/**
* A sequence of PublisherProtocolQosPolicy instances.
*/
public final class PublisherProtocolQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PublisherProtocolQosPolicySeq() {
        super(PublisherProtocolQosPolicy.class);
    }

    public PublisherProtocolQosPolicySeq (int initialMaximum) {
        super(PublisherProtocolQosPolicy.class, initialMaximum);
    }

    public PublisherProtocolQosPolicySeq (Collection elements) {
        super(PublisherProtocolQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PublisherProtocolQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PublisherProtocolQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PublisherProtocolQosPolicy.create());
