...
/**
* A sequence of WriterDataLifecycleQosPolicy instances.
*/
public final class WriterDataLifecycleQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public WriterDataLifecycleQosPolicySeq() {
        super(WriterDataLifecycleQosPolicy.class);
    }

    public WriterDataLifecycleQosPolicySeq (int initialMaximum) {
        super(WriterDataLifecycleQosPolicy.class, initialMaximum);
    }

    public WriterDataLifecycleQosPolicySeq (Collection elements) {
        super(WriterDataLifecycleQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, WriterDataLifecycleQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' WriterDataLifecycleQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(WriterDataLifecycleQosPolicy.create());
