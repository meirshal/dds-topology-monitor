...
/**
* A sequence of TransportMulticastSettings_t instances.
*/
public final class TransportMulticastSettings_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportMulticastSettings_tSeq() {
        super(TransportMulticastSettings_t.class);
    }

    public TransportMulticastSettings_tSeq (int initialMaximum) {
        super(TransportMulticastSettings_t.class, initialMaximum);
    }

    public TransportMulticastSettings_tSeq (Collection elements) {
        super(TransportMulticastSettings_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportMulticastSettings_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportMulticastSettings_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportMulticastSettings_t.create());
