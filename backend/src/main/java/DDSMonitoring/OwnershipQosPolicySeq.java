...
/**
* A sequence of OwnershipQosPolicy instances.
*/
public final class OwnershipQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OwnershipQosPolicySeq() {
        super(OwnershipQosPolicy.class);
    }

    public OwnershipQosPolicySeq (int initialMaximum) {
        super(OwnershipQosPolicy.class, initialMaximum);
    }

    public OwnershipQosPolicySeq (Collection elements) {
        super(OwnershipQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OwnershipQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OwnershipQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(OwnershipQosPolicy.create());
