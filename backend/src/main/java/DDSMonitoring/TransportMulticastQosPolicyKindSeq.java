...
/**
* A sequence of TransportMulticastQosPolicyKind instances.
*/
public final class TransportMulticastQosPolicyKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportMulticastQosPolicyKindSeq() {
        super(TransportMulticastQosPolicyKind.class);
    }

    public TransportMulticastQosPolicyKindSeq (int initialMaximum) {
        super(TransportMulticastQosPolicyKind.class, initialMaximum);
    }

    public TransportMulticastQosPolicyKindSeq (Collection elements) {
        super(TransportMulticastQosPolicyKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportMulticastQosPolicyKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportMulticastQosPolicyKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportMulticastQosPolicyKind.create());
