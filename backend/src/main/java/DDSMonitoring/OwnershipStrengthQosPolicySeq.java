...
/**
* A sequence of OwnershipStrengthQosPolicy instances.
*/
public final class OwnershipStrengthQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OwnershipStrengthQosPolicySeq() {
        super(OwnershipStrengthQosPolicy.class);
    }

    public OwnershipStrengthQosPolicySeq (int initialMaximum) {
        super(OwnershipStrengthQosPolicy.class, initialMaximum);
    }

    public OwnershipStrengthQosPolicySeq (Collection elements) {
        super(OwnershipStrengthQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OwnershipStrengthQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OwnershipStrengthQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(OwnershipStrengthQosPolicy.create());
