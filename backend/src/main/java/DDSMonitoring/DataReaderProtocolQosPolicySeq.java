...
/**
* A sequence of DataReaderProtocolQosPolicy instances.
*/
public final class DataReaderProtocolQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataReaderProtocolQosPolicySeq() {
        super(DataReaderProtocolQosPolicy.class);
    }

    public DataReaderProtocolQosPolicySeq (int initialMaximum) {
        super(DataReaderProtocolQosPolicy.class, initialMaximum);
    }

    public DataReaderProtocolQosPolicySeq (Collection elements) {
        super(DataReaderProtocolQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataReaderProtocolQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataReaderProtocolQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataReaderProtocolQosPolicy.create());
