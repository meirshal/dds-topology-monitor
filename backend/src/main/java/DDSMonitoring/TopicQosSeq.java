...
/**
* A sequence of TopicQos instances.
*/
public final class TopicQosSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TopicQosSeq() {
        super(TopicQos.class);
    }

    public TopicQosSeq (int initialMaximum) {
        super(TopicQos.class, initialMaximum);
    }

    public TopicQosSeq (Collection elements) {
        super(TopicQos.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TopicQos.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TopicQos objects (beyond the original size of this object)
