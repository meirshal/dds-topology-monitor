...
/**
* A sequence of DataWriterResourceLimitsQosPolicy instances.
*/
public final class DataWriterResourceLimitsQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataWriterResourceLimitsQosPolicySeq() {
        super(DataWriterResourceLimitsQosPolicy.class);
    }

    public DataWriterResourceLimitsQosPolicySeq (int initialMaximum) {
        super(DataWriterResourceLimitsQosPolicy.class, initialMaximum);
    }

    public DataWriterResourceLimitsQosPolicySeq (Collection elements) {
        super(DataWriterResourceLimitsQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataWriterResourceLimitsQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataWriterResourceLimitsQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataWriterResourceLimitsQosPolicy.create());
