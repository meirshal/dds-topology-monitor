...
/**
* A sequence of PublishModeQosPolicyKind instances.
*/
public final class PublishModeQosPolicyKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PublishModeQosPolicyKindSeq() {
        super(PublishModeQosPolicyKind.class);
    }

    public PublishModeQosPolicyKindSeq (int initialMaximum) {
        super(PublishModeQosPolicyKind.class, initialMaximum);
    }

    public PublishModeQosPolicyKindSeq (Collection elements) {
        super(PublishModeQosPolicyKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PublishModeQosPolicyKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PublishModeQosPolicyKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PublishModeQosPolicyKind.create());
