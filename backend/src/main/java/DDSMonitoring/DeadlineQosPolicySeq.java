...
/**
* A sequence of DeadlineQosPolicy instances.
*/
public final class DeadlineQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DeadlineQosPolicySeq() {
        super(DeadlineQosPolicy.class);
    }

    public DeadlineQosPolicySeq (int initialMaximum) {
        super(DeadlineQosPolicy.class, initialMaximum);
    }

    public DeadlineQosPolicySeq (Collection elements) {
        super(DeadlineQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DeadlineQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DeadlineQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DeadlineQosPolicy.create());
