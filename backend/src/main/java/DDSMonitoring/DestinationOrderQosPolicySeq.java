...
/**
* A sequence of DestinationOrderQosPolicy instances.
*/
public final class DestinationOrderQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DestinationOrderQosPolicySeq() {
        super(DestinationOrderQosPolicy.class);
    }

    public DestinationOrderQosPolicySeq (int initialMaximum) {
        super(DestinationOrderQosPolicy.class, initialMaximum);
    }

    public DestinationOrderQosPolicySeq (Collection elements) {
        super(DestinationOrderQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DestinationOrderQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DestinationOrderQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DestinationOrderQosPolicy.create());
