...
/**
* A sequence of MultiChannelQosPolicy instances.
*/
public final class MultiChannelQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MultiChannelQosPolicySeq() {
        super(MultiChannelQosPolicy.class);
    }

    public MultiChannelQosPolicySeq (int initialMaximum) {
        super(MultiChannelQosPolicy.class, initialMaximum);
    }

    public MultiChannelQosPolicySeq (Collection elements) {
        super(MultiChannelQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MultiChannelQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MultiChannelQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MultiChannelQosPolicy.create());
