...
/**
* A sequence of RefilterQosPolicyKind instances.
*/
public final class RefilterQosPolicyKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RefilterQosPolicyKindSeq() {
        super(RefilterQosPolicyKind.class);
    }

    public RefilterQosPolicyKindSeq (int initialMaximum) {
        super(RefilterQosPolicyKind.class, initialMaximum);
    }

    public RefilterQosPolicyKindSeq (Collection elements) {
        super(RefilterQosPolicyKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RefilterQosPolicyKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RefilterQosPolicyKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RefilterQosPolicyKind.create());
