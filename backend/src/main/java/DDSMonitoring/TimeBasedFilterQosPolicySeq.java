...
/**
* A sequence of TimeBasedFilterQosPolicy instances.
*/
public final class TimeBasedFilterQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TimeBasedFilterQosPolicySeq() {
        super(TimeBasedFilterQosPolicy.class);
    }

    public TimeBasedFilterQosPolicySeq (int initialMaximum) {
        super(TimeBasedFilterQosPolicy.class, initialMaximum);
    }

    public TimeBasedFilterQosPolicySeq (Collection elements) {
        super(TimeBasedFilterQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TimeBasedFilterQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TimeBasedFilterQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TimeBasedFilterQosPolicy.create());
