...
/**
* A sequence of DataReaderResourceLimitsQosPolicy instances.
*/
public final class DataReaderResourceLimitsQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataReaderResourceLimitsQosPolicySeq() {
        super(DataReaderResourceLimitsQosPolicy.class);
    }

    public DataReaderResourceLimitsQosPolicySeq (int initialMaximum) {
        super(DataReaderResourceLimitsQosPolicy.class, initialMaximum);
    }

    public DataReaderResourceLimitsQosPolicySeq (Collection elements) {
        super(DataReaderResourceLimitsQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataReaderResourceLimitsQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataReaderResourceLimitsQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataReaderResourceLimitsQosPolicy.create());
