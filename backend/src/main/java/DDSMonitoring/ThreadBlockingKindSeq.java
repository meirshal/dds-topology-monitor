...
/**
* A sequence of ThreadBlockingKind instances.
*/
public final class ThreadBlockingKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ThreadBlockingKindSeq() {
        super(ThreadBlockingKind.class);
    }

    public ThreadBlockingKindSeq (int initialMaximum) {
        super(ThreadBlockingKind.class, initialMaximum);
    }

    public ThreadBlockingKindSeq (Collection elements) {
        super(ThreadBlockingKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ThreadBlockingKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ThreadBlockingKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ThreadBlockingKind.create());
