...
/**
* A sequence of TopicDataQosPolicy instances.
*/
public final class TopicDataQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TopicDataQosPolicySeq() {
        super(TopicDataQosPolicy.class);
    }

    public TopicDataQosPolicySeq (int initialMaximum) {
        super(TopicDataQosPolicy.class, initialMaximum);
    }

    public TopicDataQosPolicySeq (Collection elements) {
        super(TopicDataQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TopicDataQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TopicDataQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TopicDataQosPolicy.create());
