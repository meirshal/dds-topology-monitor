...
/**
* A sequence of DataReaderCacheStatus instances.
*/
public final class DataReaderCacheStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataReaderCacheStatusSeq() {
        super(DataReaderCacheStatus.class);
    }

    public DataReaderCacheStatusSeq (int initialMaximum) {
        super(DataReaderCacheStatus.class, initialMaximum);
    }

    public DataReaderCacheStatusSeq (Collection elements) {
        super(DataReaderCacheStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataReaderCacheStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataReaderCacheStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataReaderCacheStatus.create());
