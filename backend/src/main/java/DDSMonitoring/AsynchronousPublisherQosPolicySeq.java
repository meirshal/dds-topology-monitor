...
/**
* A sequence of AsynchronousPublisherQosPolicy instances.
*/
public final class AsynchronousPublisherQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public AsynchronousPublisherQosPolicySeq() {
        super(AsynchronousPublisherQosPolicy.class);
    }

    public AsynchronousPublisherQosPolicySeq (int initialMaximum) {
        super(AsynchronousPublisherQosPolicy.class, initialMaximum);
    }

    public AsynchronousPublisherQosPolicySeq (Collection elements) {
        super(AsynchronousPublisherQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, AsynchronousPublisherQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' AsynchronousPublisherQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(AsynchronousPublisherQosPolicy.create());
