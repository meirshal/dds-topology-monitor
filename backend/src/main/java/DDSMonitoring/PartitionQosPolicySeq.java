...
/**
* A sequence of PartitionQosPolicy instances.
*/
public final class PartitionQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PartitionQosPolicySeq() {
        super(PartitionQosPolicy.class);
    }

    public PartitionQosPolicySeq (int initialMaximum) {
        super(PartitionQosPolicy.class, initialMaximum);
    }

    public PartitionQosPolicySeq (Collection elements) {
        super(PartitionQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PartitionQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PartitionQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PartitionQosPolicy.create());
