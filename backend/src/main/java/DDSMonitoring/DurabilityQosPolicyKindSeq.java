...
/**
* A sequence of DurabilityQosPolicyKind instances.
*/
public final class DurabilityQosPolicyKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DurabilityQosPolicyKindSeq() {
        super(DurabilityQosPolicyKind.class);
    }

    public DurabilityQosPolicyKindSeq (int initialMaximum) {
        super(DurabilityQosPolicyKind.class, initialMaximum);
    }

    public DurabilityQosPolicyKindSeq (Collection elements) {
        super(DurabilityQosPolicyKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DurabilityQosPolicyKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DurabilityQosPolicyKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DurabilityQosPolicyKind.create());
