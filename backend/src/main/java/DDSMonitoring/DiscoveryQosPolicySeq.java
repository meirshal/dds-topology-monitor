...
/**
* A sequence of DiscoveryQosPolicy instances.
*/
public final class DiscoveryQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DiscoveryQosPolicySeq() {
        super(DiscoveryQosPolicy.class);
    }

    public DiscoveryQosPolicySeq (int initialMaximum) {
        super(DiscoveryQosPolicy.class, initialMaximum);
    }

    public DiscoveryQosPolicySeq (Collection elements) {
        super(DiscoveryQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DiscoveryQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DiscoveryQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DiscoveryQosPolicy.create());
