...
/**
* A sequence of HistoryQosPolicyKind instances.
*/
public final class HistoryQosPolicyKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public HistoryQosPolicyKindSeq() {
        super(HistoryQosPolicyKind.class);
    }

    public HistoryQosPolicyKindSeq (int initialMaximum) {
        super(HistoryQosPolicyKind.class, initialMaximum);
    }

    public HistoryQosPolicyKindSeq (Collection elements) {
        super(HistoryQosPolicyKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, HistoryQosPolicyKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' HistoryQosPolicyKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(HistoryQosPolicyKind.create());
