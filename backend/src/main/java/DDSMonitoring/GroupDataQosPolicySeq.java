...
/**
* A sequence of GroupDataQosPolicy instances.
*/
public final class GroupDataQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GroupDataQosPolicySeq() {
        super(GroupDataQosPolicy.class);
    }

    public GroupDataQosPolicySeq (int initialMaximum) {
        super(GroupDataQosPolicy.class, initialMaximum);
    }

    public GroupDataQosPolicySeq (Collection elements) {
        super(GroupDataQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GroupDataQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GroupDataQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GroupDataQosPolicy.create());
