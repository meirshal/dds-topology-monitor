...
/**
* A sequence of PublicationMatchedStatus instances.
*/
public final class PublicationMatchedStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PublicationMatchedStatusSeq() {
        super(PublicationMatchedStatus.class);
    }

    public PublicationMatchedStatusSeq (int initialMaximum) {
        super(PublicationMatchedStatus.class, initialMaximum);
    }

    public PublicationMatchedStatusSeq (Collection elements) {
        super(PublicationMatchedStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PublicationMatchedStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PublicationMatchedStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PublicationMatchedStatus.create());
