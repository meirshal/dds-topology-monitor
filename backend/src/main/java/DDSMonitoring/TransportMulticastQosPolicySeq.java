...
/**
* A sequence of TransportMulticastQosPolicy instances.
*/
public final class TransportMulticastQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportMulticastQosPolicySeq() {
        super(TransportMulticastQosPolicy.class);
    }

    public TransportMulticastQosPolicySeq (int initialMaximum) {
        super(TransportMulticastQosPolicy.class, initialMaximum);
    }

    public TransportMulticastQosPolicySeq (Collection elements) {
        super(TransportMulticastQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportMulticastQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportMulticastQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportMulticastQosPolicy.create());
