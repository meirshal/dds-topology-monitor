...
/**
* A sequence of ThreadSettingsCpuRotationKind instances.
*/
public final class ThreadSettingsCpuRotationKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ThreadSettingsCpuRotationKindSeq() {
        super(ThreadSettingsCpuRotationKind.class);
    }

    public ThreadSettingsCpuRotationKindSeq (int initialMaximum) {
        super(ThreadSettingsCpuRotationKind.class, initialMaximum);
    }

    public ThreadSettingsCpuRotationKindSeq (Collection elements) {
        super(ThreadSettingsCpuRotationKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ThreadSettingsCpuRotationKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ThreadSettingsCpuRotationKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ThreadSettingsCpuRotationKind.create());
