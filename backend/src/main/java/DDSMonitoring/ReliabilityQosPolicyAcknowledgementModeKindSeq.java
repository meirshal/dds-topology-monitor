...
/**
* A sequence of ReliabilityQosPolicyAcknowledgementModeKind instances.
*/
public final class ReliabilityQosPolicyAcknowledgementModeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ReliabilityQosPolicyAcknowledgementModeKindSeq() {
        super(ReliabilityQosPolicyAcknowledgementModeKind.class);
    }

    public ReliabilityQosPolicyAcknowledgementModeKindSeq (int initialMaximum) {
        super(ReliabilityQosPolicyAcknowledgementModeKind.class, initialMaximum);
    }

    public ReliabilityQosPolicyAcknowledgementModeKindSeq (Collection elements) {
        super(ReliabilityQosPolicyAcknowledgementModeKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ReliabilityQosPolicyAcknowledgementModeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ReliabilityQosPolicyAcknowledgementModeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ReliabilityQosPolicyAcknowledgementModeKind.create());
