...
/**
* A sequence of DomainParticipantProtocolQosPolicy instances.
*/
public final class DomainParticipantProtocolQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DomainParticipantProtocolQosPolicySeq() {
        super(DomainParticipantProtocolQosPolicy.class);
    }

    public DomainParticipantProtocolQosPolicySeq (int initialMaximum) {
        super(DomainParticipantProtocolQosPolicy.class, initialMaximum);
    }

    public DomainParticipantProtocolQosPolicySeq (Collection elements) {
        super(DomainParticipantProtocolQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DomainParticipantProtocolQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DomainParticipantProtocolQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DomainParticipantProtocolQosPolicy.create());
