...
/**
* A sequence of TypeConsistencyEnforcementQosPolicy instances.
*/
public final class TypeConsistencyEnforcementQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TypeConsistencyEnforcementQosPolicySeq() {
        super(TypeConsistencyEnforcementQosPolicy.class);
    }

    public TypeConsistencyEnforcementQosPolicySeq (int initialMaximum) {
        super(TypeConsistencyEnforcementQosPolicy.class, initialMaximum);
    }

    public TypeConsistencyEnforcementQosPolicySeq (Collection elements) {
        super(TypeConsistencyEnforcementQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TypeConsistencyEnforcementQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TypeConsistencyEnforcementQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TypeConsistencyEnforcementQosPolicy.create());
