...
/**
* A sequence of OwnershipQosPolicyKind instances.
*/
public final class OwnershipQosPolicyKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OwnershipQosPolicyKindSeq() {
        super(OwnershipQosPolicyKind.class);
    }

    public OwnershipQosPolicyKindSeq (int initialMaximum) {
        super(OwnershipQosPolicyKind.class, initialMaximum);
    }

    public OwnershipQosPolicyKindSeq (Collection elements) {
        super(OwnershipQosPolicyKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OwnershipQosPolicyKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OwnershipQosPolicyKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(OwnershipQosPolicyKind.create());
