...
/**
* A sequence of ChannelSettings_t instances.
*/
public final class ChannelSettings_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ChannelSettings_tSeq() {
        super(ChannelSettings_t.class);
    }

    public ChannelSettings_tSeq (int initialMaximum) {
        super(ChannelSettings_t.class, initialMaximum);
    }

    public ChannelSettings_tSeq (Collection elements) {
        super(ChannelSettings_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ChannelSettings_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ChannelSettings_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ChannelSettings_t.create());
