...
/**
* A sequence of TransportMulticastMapping_t instances.
*/
public final class TransportMulticastMapping_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportMulticastMapping_tSeq() {
        super(TransportMulticastMapping_t.class);
    }

    public TransportMulticastMapping_tSeq (int initialMaximum) {
        super(TransportMulticastMapping_t.class, initialMaximum);
    }

    public TransportMulticastMapping_tSeq (Collection elements) {
        super(TransportMulticastMapping_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportMulticastMapping_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportMulticastMapping_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportMulticastMapping_t.create());
