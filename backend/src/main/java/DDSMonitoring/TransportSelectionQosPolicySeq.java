...
/**
* A sequence of TransportSelectionQosPolicy instances.
*/
public final class TransportSelectionQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportSelectionQosPolicySeq() {
        super(TransportSelectionQosPolicy.class);
    }

    public TransportSelectionQosPolicySeq (int initialMaximum) {
        super(TransportSelectionQosPolicy.class, initialMaximum);
    }

    public TransportSelectionQosPolicySeq (Collection elements) {
        super(TransportSelectionQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportSelectionQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportSelectionQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportSelectionQosPolicy.create());
