...
/**
* A sequence of ReaderDataLifecycleQosPolicy instances.
*/
public final class ReaderDataLifecycleQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ReaderDataLifecycleQosPolicySeq() {
        super(ReaderDataLifecycleQosPolicy.class);
    }

    public ReaderDataLifecycleQosPolicySeq (int initialMaximum) {
        super(ReaderDataLifecycleQosPolicy.class, initialMaximum);
    }

    public ReaderDataLifecycleQosPolicySeq (Collection elements) {
        super(ReaderDataLifecycleQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ReaderDataLifecycleQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ReaderDataLifecycleQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ReaderDataLifecycleQosPolicy.create());
