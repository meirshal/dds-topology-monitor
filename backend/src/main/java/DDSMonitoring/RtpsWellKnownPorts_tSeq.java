...
/**
* A sequence of RtpsWellKnownPorts_t instances.
*/
public final class RtpsWellKnownPorts_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RtpsWellKnownPorts_tSeq() {
        super(RtpsWellKnownPorts_t.class);
    }

    public RtpsWellKnownPorts_tSeq (int initialMaximum) {
        super(RtpsWellKnownPorts_t.class, initialMaximum);
    }

    public RtpsWellKnownPorts_tSeq (Collection elements) {
        super(RtpsWellKnownPorts_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RtpsWellKnownPorts_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RtpsWellKnownPorts_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RtpsWellKnownPorts_t.create());
