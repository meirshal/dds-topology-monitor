...
/**
* A sequence of SampleRejectedStatusKind instances.
*/
public final class SampleRejectedStatusKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SampleRejectedStatusKindSeq() {
        super(SampleRejectedStatusKind.class);
    }

    public SampleRejectedStatusKindSeq (int initialMaximum) {
        super(SampleRejectedStatusKind.class, initialMaximum);
    }

    public SampleRejectedStatusKindSeq (Collection elements) {
        super(SampleRejectedStatusKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SampleRejectedStatusKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SampleRejectedStatusKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SampleRejectedStatusKind.create());
