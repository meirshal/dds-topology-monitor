...
/**
* A sequence of DomainParticipantResourceLimitsQosPolicy instances.
*/
public final class DomainParticipantResourceLimitsQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DomainParticipantResourceLimitsQosPolicySeq() {
        super(DomainParticipantResourceLimitsQosPolicy.class);
    }

    public DomainParticipantResourceLimitsQosPolicySeq (int initialMaximum) {
        super(DomainParticipantResourceLimitsQosPolicy.class, initialMaximum);
    }

    public DomainParticipantResourceLimitsQosPolicySeq (Collection elements) {
        super(DomainParticipantResourceLimitsQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DomainParticipantResourceLimitsQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DomainParticipantResourceLimitsQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DomainParticipantResourceLimitsQosPolicy.create());
