...
/**
* A sequence of TransportBuiltinQosPolicy instances.
*/
public final class TransportBuiltinQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportBuiltinQosPolicySeq() {
        super(TransportBuiltinQosPolicy.class);
    }

    public TransportBuiltinQosPolicySeq (int initialMaximum) {
        super(TransportBuiltinQosPolicy.class, initialMaximum);
    }

    public TransportBuiltinQosPolicySeq (Collection elements) {
        super(TransportBuiltinQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportBuiltinQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportBuiltinQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportBuiltinQosPolicy.create());
