...
/**
* A sequence of SampleRejectedStatus instances.
*/
public final class SampleRejectedStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SampleRejectedStatusSeq() {
        super(SampleRejectedStatus.class);
    }

    public SampleRejectedStatusSeq (int initialMaximum) {
        super(SampleRejectedStatus.class, initialMaximum);
    }

    public SampleRejectedStatusSeq (Collection elements) {
        super(SampleRejectedStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SampleRejectedStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SampleRejectedStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SampleRejectedStatus.create());
