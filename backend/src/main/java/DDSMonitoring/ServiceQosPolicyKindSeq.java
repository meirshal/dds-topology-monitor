...
/**
* A sequence of ServiceQosPolicyKind instances.
*/
public final class ServiceQosPolicyKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ServiceQosPolicyKindSeq() {
        super(ServiceQosPolicyKind.class);
    }

    public ServiceQosPolicyKindSeq (int initialMaximum) {
        super(ServiceQosPolicyKind.class, initialMaximum);
    }

    public ServiceQosPolicyKindSeq (Collection elements) {
        super(ServiceQosPolicyKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ServiceQosPolicyKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ServiceQosPolicyKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ServiceQosPolicyKind.create());
