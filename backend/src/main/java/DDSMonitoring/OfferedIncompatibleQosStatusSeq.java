...
/**
* A sequence of OfferedIncompatibleQosStatus instances.
*/
public final class OfferedIncompatibleQosStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OfferedIncompatibleQosStatusSeq() {
        super(OfferedIncompatibleQosStatus.class);
    }

    public OfferedIncompatibleQosStatusSeq (int initialMaximum) {
        super(OfferedIncompatibleQosStatus.class, initialMaximum);
    }

    public OfferedIncompatibleQosStatusSeq (Collection elements) {
        super(OfferedIncompatibleQosStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OfferedIncompatibleQosStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OfferedIncompatibleQosStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(OfferedIncompatibleQosStatus.create());
