...
/**
* A sequence of TypeConsistencyKind instances.
*/
public final class TypeConsistencyKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TypeConsistencyKindSeq() {
        super(TypeConsistencyKind.class);
    }

    public TypeConsistencyKindSeq (int initialMaximum) {
        super(TypeConsistencyKind.class, initialMaximum);
    }

    public TypeConsistencyKindSeq (Collection elements) {
        super(TypeConsistencyKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TypeConsistencyKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TypeConsistencyKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TypeConsistencyKind.create());
