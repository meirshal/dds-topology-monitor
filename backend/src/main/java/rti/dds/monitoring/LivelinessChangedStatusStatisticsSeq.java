...
/**
* A sequence of LivelinessChangedStatusStatistics instances.
*/
public final class LivelinessChangedStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LivelinessChangedStatusStatisticsSeq() {
        super(LivelinessChangedStatusStatistics.class);
    }

    public LivelinessChangedStatusStatisticsSeq (int initialMaximum) {
        super(LivelinessChangedStatusStatistics.class, initialMaximum);
    }

    public LivelinessChangedStatusStatisticsSeq (Collection elements) {
        super(LivelinessChangedStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LivelinessChangedStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LivelinessChangedStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LivelinessChangedStatusStatistics.create());
