...
/**
* A sequence of DataReaderDescription instances.
*/
public final class DataReaderDescriptionSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataReaderDescriptionSeq() {
        super(DataReaderDescription.class);
    }

    public DataReaderDescriptionSeq (int initialMaximum) {
        super(DataReaderDescription.class, initialMaximum);
    }

    public DataReaderDescriptionSeq (Collection elements) {
        super(DataReaderDescription.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataReaderDescription.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataReaderDescription objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataReaderDescription.create());
