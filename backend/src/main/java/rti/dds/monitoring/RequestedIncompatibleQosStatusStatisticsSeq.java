...
/**
* A sequence of RequestedIncompatibleQosStatusStatistics instances.
*/
public final class RequestedIncompatibleQosStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RequestedIncompatibleQosStatusStatisticsSeq() {
        super(RequestedIncompatibleQosStatusStatistics.class);
    }

    public RequestedIncompatibleQosStatusStatisticsSeq (int initialMaximum) {
        super(RequestedIncompatibleQosStatusStatistics.class, initialMaximum);
    }

    public RequestedIncompatibleQosStatusStatisticsSeq (Collection elements) {
        super(RequestedIncompatibleQosStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RequestedIncompatibleQosStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RequestedIncompatibleQosStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RequestedIncompatibleQosStatusStatistics.create());
