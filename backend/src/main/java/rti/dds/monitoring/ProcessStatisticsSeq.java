...
/**
* A sequence of ProcessStatistics instances.
*/
public final class ProcessStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ProcessStatisticsSeq() {
        super(ProcessStatistics.class);
    }

    public ProcessStatisticsSeq (int initialMaximum) {
        super(ProcessStatistics.class, initialMaximum);
    }

    public ProcessStatisticsSeq (Collection elements) {
        super(ProcessStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ProcessStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ProcessStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ProcessStatistics.create());
