...
/**
* A sequence of DataReaderEntityStatistics instances.
*/
public final class DataReaderEntityStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataReaderEntityStatisticsSeq() {
        super(DataReaderEntityStatistics.class);
    }

    public DataReaderEntityStatisticsSeq (int initialMaximum) {
        super(DataReaderEntityStatistics.class, initialMaximum);
    }

    public DataReaderEntityStatisticsSeq (Collection elements) {
        super(DataReaderEntityStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataReaderEntityStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataReaderEntityStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataReaderEntityStatistics.create());
