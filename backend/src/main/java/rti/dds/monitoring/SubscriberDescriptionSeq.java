...
/**
* A sequence of SubscriberDescription instances.
*/
public final class SubscriberDescriptionSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SubscriberDescriptionSeq() {
        super(SubscriberDescription.class);
    }

    public SubscriberDescriptionSeq (int initialMaximum) {
        super(SubscriberDescription.class, initialMaximum);
    }

    public SubscriberDescriptionSeq (Collection elements) {
        super(SubscriberDescription.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SubscriberDescription.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SubscriberDescription objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SubscriberDescription.create());
