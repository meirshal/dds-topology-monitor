...
/**
* A sequence of RequestedDeadlineMissedStatusStatistics instances.
*/
public final class RequestedDeadlineMissedStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RequestedDeadlineMissedStatusStatisticsSeq() {
        super(RequestedDeadlineMissedStatusStatistics.class);
    }

    public RequestedDeadlineMissedStatusStatisticsSeq (int initialMaximum) {
        super(RequestedDeadlineMissedStatusStatistics.class, initialMaximum);
    }

    public RequestedDeadlineMissedStatusStatisticsSeq (Collection elements) {
        super(RequestedDeadlineMissedStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RequestedDeadlineMissedStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RequestedDeadlineMissedStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RequestedDeadlineMissedStatusStatistics.create());
