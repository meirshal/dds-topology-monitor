...
/**
* A sequence of ReliableWriterCacheChangedStatusStatistics instances.
*/
public final class ReliableWriterCacheChangedStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ReliableWriterCacheChangedStatusStatisticsSeq() {
        super(ReliableWriterCacheChangedStatusStatistics.class);
    }

    public ReliableWriterCacheChangedStatusStatisticsSeq (int initialMaximum) {
        super(ReliableWriterCacheChangedStatusStatistics.class, initialMaximum);
    }

    public ReliableWriterCacheChangedStatusStatisticsSeq (Collection elements) {
        super(ReliableWriterCacheChangedStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ReliableWriterCacheChangedStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ReliableWriterCacheChangedStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ReliableWriterCacheChangedStatusStatistics.create());
