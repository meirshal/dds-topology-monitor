...
/**
* A sequence of DataWriterDescription instances.
*/
public final class DataWriterDescriptionSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataWriterDescriptionSeq() {
        super(DataWriterDescription.class);
    }

    public DataWriterDescriptionSeq (int initialMaximum) {
        super(DataWriterDescription.class, initialMaximum);
    }

    public DataWriterDescriptionSeq (Collection elements) {
        super(DataWriterDescription.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataWriterDescription.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataWriterDescription objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataWriterDescription.create());
