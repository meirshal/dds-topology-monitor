...
/**
* A sequence of SampleLostStatusStatistics instances.
*/
public final class SampleLostStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SampleLostStatusStatisticsSeq() {
        super(SampleLostStatusStatistics.class);
    }

    public SampleLostStatusStatisticsSeq (int initialMaximum) {
        super(SampleLostStatusStatistics.class, initialMaximum);
    }

    public SampleLostStatusStatisticsSeq (Collection elements) {
        super(SampleLostStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SampleLostStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SampleLostStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SampleLostStatusStatistics.create());
