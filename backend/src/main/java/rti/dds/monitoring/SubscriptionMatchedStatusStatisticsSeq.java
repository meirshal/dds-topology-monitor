...
/**
* A sequence of SubscriptionMatchedStatusStatistics instances.
*/
public final class SubscriptionMatchedStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SubscriptionMatchedStatusStatisticsSeq() {
        super(SubscriptionMatchedStatusStatistics.class);
    }

    public SubscriptionMatchedStatusStatisticsSeq (int initialMaximum) {
        super(SubscriptionMatchedStatusStatistics.class, initialMaximum);
    }

    public SubscriptionMatchedStatusStatisticsSeq (Collection elements) {
        super(SubscriptionMatchedStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SubscriptionMatchedStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SubscriptionMatchedStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SubscriptionMatchedStatusStatistics.create());
