...
/**
* A sequence of DataReaderCacheStatusStatistics instances.
*/
public final class DataReaderCacheStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataReaderCacheStatusStatisticsSeq() {
        super(DataReaderCacheStatusStatistics.class);
    }

    public DataReaderCacheStatusStatisticsSeq (int initialMaximum) {
        super(DataReaderCacheStatusStatistics.class, initialMaximum);
    }

    public DataReaderCacheStatusStatisticsSeq (Collection elements) {
        super(DataReaderCacheStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataReaderCacheStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataReaderCacheStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataReaderCacheStatusStatistics.create());
