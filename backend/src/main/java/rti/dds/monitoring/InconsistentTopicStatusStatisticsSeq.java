...
/**
* A sequence of InconsistentTopicStatusStatistics instances.
*/
public final class InconsistentTopicStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public InconsistentTopicStatusStatisticsSeq() {
        super(InconsistentTopicStatusStatistics.class);
    }

    public InconsistentTopicStatusStatisticsSeq (int initialMaximum) {
        super(InconsistentTopicStatusStatistics.class, initialMaximum);
    }

    public InconsistentTopicStatusStatisticsSeq (Collection elements) {
        super(InconsistentTopicStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, InconsistentTopicStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' InconsistentTopicStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(InconsistentTopicStatusStatistics.create());
