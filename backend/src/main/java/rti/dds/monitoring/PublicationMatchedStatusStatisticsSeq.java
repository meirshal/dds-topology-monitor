...
/**
* A sequence of PublicationMatchedStatusStatistics instances.
*/
public final class PublicationMatchedStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PublicationMatchedStatusStatisticsSeq() {
        super(PublicationMatchedStatusStatistics.class);
    }

    public PublicationMatchedStatusStatisticsSeq (int initialMaximum) {
        super(PublicationMatchedStatusStatistics.class, initialMaximum);
    }

    public PublicationMatchedStatusStatisticsSeq (Collection elements) {
        super(PublicationMatchedStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PublicationMatchedStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PublicationMatchedStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PublicationMatchedStatusStatistics.create());
