...
/**
* A sequence of DataReaderProtocolStatusStatistics instances.
*/
public final class DataReaderProtocolStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataReaderProtocolStatusStatisticsSeq() {
        super(DataReaderProtocolStatusStatistics.class);
    }

    public DataReaderProtocolStatusStatisticsSeq (int initialMaximum) {
        super(DataReaderProtocolStatusStatistics.class, initialMaximum);
    }

    public DataReaderProtocolStatusStatisticsSeq (Collection elements) {
        super(DataReaderProtocolStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataReaderProtocolStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataReaderProtocolStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataReaderProtocolStatusStatistics.create());
