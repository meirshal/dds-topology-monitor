...
/**
* A sequence of OfferedIncompatibleQosStatusStatistics instances.
*/
public final class OfferedIncompatibleQosStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OfferedIncompatibleQosStatusStatisticsSeq() {
        super(OfferedIncompatibleQosStatusStatistics.class);
    }

    public OfferedIncompatibleQosStatusStatisticsSeq (int initialMaximum) {
        super(OfferedIncompatibleQosStatusStatistics.class, initialMaximum);
    }

    public OfferedIncompatibleQosStatusStatisticsSeq (Collection elements) {
        super(OfferedIncompatibleQosStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OfferedIncompatibleQosStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OfferedIncompatibleQosStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(OfferedIncompatibleQosStatusStatistics.create());
