...
/**
* A sequence of PublisherDescription instances.
*/
public final class PublisherDescriptionSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PublisherDescriptionSeq() {
        super(PublisherDescription.class);
    }

    public PublisherDescriptionSeq (int initialMaximum) {
        super(PublisherDescription.class, initialMaximum);
    }

    public PublisherDescriptionSeq (Collection elements) {
        super(PublisherDescription.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PublisherDescription.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PublisherDescription objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PublisherDescription.create());
