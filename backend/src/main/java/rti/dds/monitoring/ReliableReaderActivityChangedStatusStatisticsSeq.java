...
/**
* A sequence of ReliableReaderActivityChangedStatusStatistics instances.
*/
public final class ReliableReaderActivityChangedStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ReliableReaderActivityChangedStatusStatisticsSeq() {
        super(ReliableReaderActivityChangedStatusStatistics.class);
    }

    public ReliableReaderActivityChangedStatusStatisticsSeq (int initialMaximum) {
        super(ReliableReaderActivityChangedStatusStatistics.class, initialMaximum);
    }

    public ReliableReaderActivityChangedStatusStatisticsSeq (Collection elements) {
        super(ReliableReaderActivityChangedStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ReliableReaderActivityChangedStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ReliableReaderActivityChangedStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ReliableReaderActivityChangedStatusStatistics.create());
