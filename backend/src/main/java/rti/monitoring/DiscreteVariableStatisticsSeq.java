...
/**
* A sequence of DiscreteVariableStatistics instances.
*/
public final class DiscreteVariableStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DiscreteVariableStatisticsSeq() {
        super(DiscreteVariableStatistics.class);
    }

    public DiscreteVariableStatisticsSeq (int initialMaximum) {
        super(DiscreteVariableStatistics.class, initialMaximum);
    }

    public DiscreteVariableStatisticsSeq (Collection elements) {
        super(DiscreteVariableStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DiscreteVariableStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DiscreteVariableStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DiscreteVariableStatistics.create());
