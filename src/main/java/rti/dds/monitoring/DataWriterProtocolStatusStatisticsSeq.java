...
/**
* A sequence of DataWriterProtocolStatusStatistics instances.
*/
public final class DataWriterProtocolStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataWriterProtocolStatusStatisticsSeq() {
        super(DataWriterProtocolStatusStatistics.class);
    }

    public DataWriterProtocolStatusStatisticsSeq (int initialMaximum) {
        super(DataWriterProtocolStatusStatistics.class, initialMaximum);
    }

    public DataWriterProtocolStatusStatisticsSeq (Collection elements) {
        super(DataWriterProtocolStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataWriterProtocolStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataWriterProtocolStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataWriterProtocolStatusStatistics.create());
