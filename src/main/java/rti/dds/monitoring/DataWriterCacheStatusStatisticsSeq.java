...
/**
* A sequence of DataWriterCacheStatusStatistics instances.
*/
public final class DataWriterCacheStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataWriterCacheStatusStatisticsSeq() {
        super(DataWriterCacheStatusStatistics.class);
    }

    public DataWriterCacheStatusStatisticsSeq (int initialMaximum) {
        super(DataWriterCacheStatusStatistics.class, initialMaximum);
    }

    public DataWriterCacheStatusStatisticsSeq (Collection elements) {
        super(DataWriterCacheStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataWriterCacheStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataWriterCacheStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataWriterCacheStatusStatistics.create());
