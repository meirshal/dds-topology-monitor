...
/**
* A sequence of DataReaderEntityMatchedPublicationStatistics instances.
*/
public final class DataReaderEntityMatchedPublicationStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataReaderEntityMatchedPublicationStatisticsSeq() {
        super(DataReaderEntityMatchedPublicationStatistics.class);
    }

    public DataReaderEntityMatchedPublicationStatisticsSeq (int initialMaximum) {
        super(DataReaderEntityMatchedPublicationStatistics.class, initialMaximum);
    }

    public DataReaderEntityMatchedPublicationStatisticsSeq (Collection elements) {
        super(DataReaderEntityMatchedPublicationStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataReaderEntityMatchedPublicationStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataReaderEntityMatchedPublicationStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataReaderEntityMatchedPublicationStatistics.create());
