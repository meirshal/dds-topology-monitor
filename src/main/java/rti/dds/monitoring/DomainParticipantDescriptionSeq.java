...
/**
* A sequence of DomainParticipantDescription instances.
*/
public final class DomainParticipantDescriptionSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DomainParticipantDescriptionSeq() {
        super(DomainParticipantDescription.class);
    }

    public DomainParticipantDescriptionSeq (int initialMaximum) {
        super(DomainParticipantDescription.class, initialMaximum);
    }

    public DomainParticipantDescriptionSeq (Collection elements) {
        super(DomainParticipantDescription.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DomainParticipantDescription.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DomainParticipantDescription objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DomainParticipantDescription.create());
