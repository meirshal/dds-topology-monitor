...
/**
* A sequence of LivelinessLostStatusStatistics instances.
*/
public final class LivelinessLostStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LivelinessLostStatusStatisticsSeq() {
        super(LivelinessLostStatusStatistics.class);
    }

    public LivelinessLostStatusStatisticsSeq (int initialMaximum) {
        super(LivelinessLostStatusStatistics.class, initialMaximum);
    }

    public LivelinessLostStatusStatisticsSeq (Collection elements) {
        super(LivelinessLostStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LivelinessLostStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LivelinessLostStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LivelinessLostStatusStatistics.create());
