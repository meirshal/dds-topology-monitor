...
/**
* A sequence of DomainParticipantEntityStatistics instances.
*/
public final class DomainParticipantEntityStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DomainParticipantEntityStatisticsSeq() {
        super(DomainParticipantEntityStatistics.class);
    }

    public DomainParticipantEntityStatisticsSeq (int initialMaximum) {
        super(DomainParticipantEntityStatistics.class, initialMaximum);
    }

    public DomainParticipantEntityStatisticsSeq (Collection elements) {
        super(DomainParticipantEntityStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DomainParticipantEntityStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DomainParticipantEntityStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DomainParticipantEntityStatistics.create());
