...
/**
* A sequence of DataWriterEntityMatchedSubscriptionStatistics instances.
*/
public final class DataWriterEntityMatchedSubscriptionStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataWriterEntityMatchedSubscriptionStatisticsSeq() {
        super(DataWriterEntityMatchedSubscriptionStatistics.class);
    }

    public DataWriterEntityMatchedSubscriptionStatisticsSeq (int initialMaximum) {
        super(DataWriterEntityMatchedSubscriptionStatistics.class, initialMaximum);
    }

    public DataWriterEntityMatchedSubscriptionStatisticsSeq (Collection elements) {
        super(DataWriterEntityMatchedSubscriptionStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataWriterEntityMatchedSubscriptionStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataWriterEntityMatchedSubscriptionStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataWriterEntityMatchedSubscriptionStatistics.create());
