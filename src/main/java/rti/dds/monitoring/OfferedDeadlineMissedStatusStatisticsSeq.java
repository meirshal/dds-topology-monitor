...
/**
* A sequence of OfferedDeadlineMissedStatusStatistics instances.
*/
public final class OfferedDeadlineMissedStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OfferedDeadlineMissedStatusStatisticsSeq() {
        super(OfferedDeadlineMissedStatusStatistics.class);
    }

    public OfferedDeadlineMissedStatusStatisticsSeq (int initialMaximum) {
        super(OfferedDeadlineMissedStatusStatistics.class, initialMaximum);
    }

    public OfferedDeadlineMissedStatusStatisticsSeq (Collection elements) {
        super(OfferedDeadlineMissedStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OfferedDeadlineMissedStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OfferedDeadlineMissedStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(OfferedDeadlineMissedStatusStatistics.create());
