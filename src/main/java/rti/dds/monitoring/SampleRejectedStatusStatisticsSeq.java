...
/**
* A sequence of SampleRejectedStatusStatistics instances.
*/
public final class SampleRejectedStatusStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SampleRejectedStatusStatisticsSeq() {
        super(SampleRejectedStatusStatistics.class);
    }

    public SampleRejectedStatusStatisticsSeq (int initialMaximum) {
        super(SampleRejectedStatusStatistics.class, initialMaximum);
    }

    public SampleRejectedStatusStatisticsSeq (Collection elements) {
        super(SampleRejectedStatusStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SampleRejectedStatusStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SampleRejectedStatusStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SampleRejectedStatusStatistics.create());
