...
/**
* A sequence of TopicEntityStatistics instances.
*/
public final class TopicEntityStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TopicEntityStatisticsSeq() {
        super(TopicEntityStatistics.class);
    }

    public TopicEntityStatisticsSeq (int initialMaximum) {
        super(TopicEntityStatistics.class, initialMaximum);
    }

    public TopicEntityStatisticsSeq (Collection elements) {
        super(TopicEntityStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TopicEntityStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TopicEntityStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TopicEntityStatistics.create());
