...
/**
* A sequence of DataWriterEntityStatistics instances.
*/
public final class DataWriterEntityStatisticsSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataWriterEntityStatisticsSeq() {
        super(DataWriterEntityStatistics.class);
    }

    public DataWriterEntityStatisticsSeq (int initialMaximum) {
        super(DataWriterEntityStatistics.class, initialMaximum);
    }

    public DataWriterEntityStatisticsSeq (Collection elements) {
        super(DataWriterEntityStatistics.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataWriterEntityStatistics.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataWriterEntityStatistics objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataWriterEntityStatistics.create());
