...
/**
* A sequence of OfferedDeadlineMissedStatus instances.
*/
public final class OfferedDeadlineMissedStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OfferedDeadlineMissedStatusSeq() {
        super(OfferedDeadlineMissedStatus.class);
    }

    public OfferedDeadlineMissedStatusSeq (int initialMaximum) {
        super(OfferedDeadlineMissedStatus.class, initialMaximum);
    }

    public OfferedDeadlineMissedStatusSeq (Collection elements) {
        super(OfferedDeadlineMissedStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OfferedDeadlineMissedStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OfferedDeadlineMissedStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(OfferedDeadlineMissedStatus.create());
