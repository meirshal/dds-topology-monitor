...
/**
* A sequence of LatencyBudgetQosPolicy instances.
*/
public final class LatencyBudgetQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LatencyBudgetQosPolicySeq() {
        super(LatencyBudgetQosPolicy.class);
    }

    public LatencyBudgetQosPolicySeq (int initialMaximum) {
        super(LatencyBudgetQosPolicy.class, initialMaximum);
    }

    public LatencyBudgetQosPolicySeq (Collection elements) {
        super(LatencyBudgetQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LatencyBudgetQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LatencyBudgetQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LatencyBudgetQosPolicy.create());
