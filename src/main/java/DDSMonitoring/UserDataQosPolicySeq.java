...
/**
* A sequence of UserDataQosPolicy instances.
*/
public final class UserDataQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public UserDataQosPolicySeq() {
        super(UserDataQosPolicy.class);
    }

    public UserDataQosPolicySeq (int initialMaximum) {
        super(UserDataQosPolicy.class, initialMaximum);
    }

    public UserDataQosPolicySeq (Collection elements) {
        super(UserDataQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, UserDataQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' UserDataQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(UserDataQosPolicy.create());
