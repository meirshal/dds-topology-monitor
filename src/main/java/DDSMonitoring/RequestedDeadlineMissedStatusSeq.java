...
/**
* A sequence of RequestedDeadlineMissedStatus instances.
*/
public final class RequestedDeadlineMissedStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RequestedDeadlineMissedStatusSeq() {
        super(RequestedDeadlineMissedStatus.class);
    }

    public RequestedDeadlineMissedStatusSeq (int initialMaximum) {
        super(RequestedDeadlineMissedStatus.class, initialMaximum);
    }

    public RequestedDeadlineMissedStatusSeq (Collection elements) {
        super(RequestedDeadlineMissedStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RequestedDeadlineMissedStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RequestedDeadlineMissedStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RequestedDeadlineMissedStatus.create());
