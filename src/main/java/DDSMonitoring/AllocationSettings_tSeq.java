...
/**
* A sequence of AllocationSettings_t instances.
*/
public final class AllocationSettings_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public AllocationSettings_tSeq() {
        super(AllocationSettings_t.class);
    }

    public AllocationSettings_tSeq (int initialMaximum) {
        super(AllocationSettings_t.class, initialMaximum);
    }

    public AllocationSettings_tSeq (Collection elements) {
        super(AllocationSettings_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, AllocationSettings_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' AllocationSettings_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(AllocationSettings_t.create());
