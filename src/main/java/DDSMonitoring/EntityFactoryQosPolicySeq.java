...
/**
* A sequence of EntityFactoryQosPolicy instances.
*/
public final class EntityFactoryQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public EntityFactoryQosPolicySeq() {
        super(EntityFactoryQosPolicy.class);
    }

    public EntityFactoryQosPolicySeq (int initialMaximum) {
        super(EntityFactoryQosPolicy.class, initialMaximum);
    }

    public EntityFactoryQosPolicySeq (Collection elements) {
        super(EntityFactoryQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, EntityFactoryQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' EntityFactoryQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(EntityFactoryQosPolicy.create());
