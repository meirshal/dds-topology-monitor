...
/**
* A sequence of DestinationOrderQosPolicyScopeKind instances.
*/
public final class DestinationOrderQosPolicyScopeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DestinationOrderQosPolicyScopeKindSeq() {
        super(DestinationOrderQosPolicyScopeKind.class);
    }

    public DestinationOrderQosPolicyScopeKindSeq (int initialMaximum) {
        super(DestinationOrderQosPolicyScopeKind.class, initialMaximum);
    }

    public DestinationOrderQosPolicyScopeKindSeq (Collection elements) {
        super(DestinationOrderQosPolicyScopeKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DestinationOrderQosPolicyScopeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DestinationOrderQosPolicyScopeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DestinationOrderQosPolicyScopeKind.create());
