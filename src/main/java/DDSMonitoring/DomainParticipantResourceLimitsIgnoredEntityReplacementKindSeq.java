...
/**
* A sequence of DomainParticipantResourceLimitsIgnoredEntityReplacementKind instances.
*/
public final class DomainParticipantResourceLimitsIgnoredEntityReplacementKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DomainParticipantResourceLimitsIgnoredEntityReplacementKindSeq() {
        super(DomainParticipantResourceLimitsIgnoredEntityReplacementKind.class);
    }

    public DomainParticipantResourceLimitsIgnoredEntityReplacementKindSeq (int initialMaximum) {
        super(DomainParticipantResourceLimitsIgnoredEntityReplacementKind.class, initialMaximum);
    }

    public DomainParticipantResourceLimitsIgnoredEntityReplacementKindSeq (Collection elements) {
        super(DomainParticipantResourceLimitsIgnoredEntityReplacementKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DomainParticipantResourceLimitsIgnoredEntityReplacementKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DomainParticipantResourceLimitsIgnoredEntityReplacementKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DomainParticipantResourceLimitsIgnoredEntityReplacementKind.create());
