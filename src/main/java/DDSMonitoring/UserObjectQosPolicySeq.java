...
/**
* A sequence of UserObjectQosPolicy instances.
*/
public final class UserObjectQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public UserObjectQosPolicySeq() {
        super(UserObjectQosPolicy.class);
    }

    public UserObjectQosPolicySeq (int initialMaximum) {
        super(UserObjectQosPolicy.class, initialMaximum);
    }

    public UserObjectQosPolicySeq (Collection elements) {
        super(UserObjectQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, UserObjectQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' UserObjectQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(UserObjectQosPolicy.create());
