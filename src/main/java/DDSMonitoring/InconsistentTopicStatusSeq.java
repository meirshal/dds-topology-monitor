...
/**
* A sequence of InconsistentTopicStatus instances.
*/
public final class InconsistentTopicStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public InconsistentTopicStatusSeq() {
        super(InconsistentTopicStatus.class);
    }

    public InconsistentTopicStatusSeq (int initialMaximum) {
        super(InconsistentTopicStatus.class, initialMaximum);
    }

    public InconsistentTopicStatusSeq (Collection elements) {
        super(InconsistentTopicStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, InconsistentTopicStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' InconsistentTopicStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(InconsistentTopicStatus.create());
