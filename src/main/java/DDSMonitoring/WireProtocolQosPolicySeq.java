...
/**
* A sequence of WireProtocolQosPolicy instances.
*/
public final class WireProtocolQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public WireProtocolQosPolicySeq() {
        super(WireProtocolQosPolicy.class);
    }

    public WireProtocolQosPolicySeq (int initialMaximum) {
        super(WireProtocolQosPolicy.class, initialMaximum);
    }

    public WireProtocolQosPolicySeq (Collection elements) {
        super(WireProtocolQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, WireProtocolQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' WireProtocolQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(WireProtocolQosPolicy.create());
