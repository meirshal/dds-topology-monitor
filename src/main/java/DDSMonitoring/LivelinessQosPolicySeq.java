...
/**
* A sequence of LivelinessQosPolicy instances.
*/
public final class LivelinessQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LivelinessQosPolicySeq() {
        super(LivelinessQosPolicy.class);
    }

    public LivelinessQosPolicySeq (int initialMaximum) {
        super(LivelinessQosPolicy.class, initialMaximum);
    }

    public LivelinessQosPolicySeq (Collection elements) {
        super(LivelinessQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LivelinessQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LivelinessQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LivelinessQosPolicy.create());
