...
/**
* A sequence of TransportUnicastQosPolicy instances.
*/
public final class TransportUnicastQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportUnicastQosPolicySeq() {
        super(TransportUnicastQosPolicy.class);
    }

    public TransportUnicastQosPolicySeq (int initialMaximum) {
        super(TransportUnicastQosPolicy.class, initialMaximum);
    }

    public TransportUnicastQosPolicySeq (Collection elements) {
        super(TransportUnicastQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportUnicastQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportUnicastQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportUnicastQosPolicy.create());
