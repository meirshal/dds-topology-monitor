...
/**
* A sequence of LivelinessChangedStatus instances.
*/
public final class LivelinessChangedStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LivelinessChangedStatusSeq() {
        super(LivelinessChangedStatus.class);
    }

    public LivelinessChangedStatusSeq (int initialMaximum) {
        super(LivelinessChangedStatus.class, initialMaximum);
    }

    public LivelinessChangedStatusSeq (Collection elements) {
        super(LivelinessChangedStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LivelinessChangedStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LivelinessChangedStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LivelinessChangedStatus.create());
