...
/**
* A sequence of ReliableWriterCacheEventCount instances.
*/
public final class ReliableWriterCacheEventCountSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ReliableWriterCacheEventCountSeq() {
        super(ReliableWriterCacheEventCount.class);
    }

    public ReliableWriterCacheEventCountSeq (int initialMaximum) {
        super(ReliableWriterCacheEventCount.class, initialMaximum);
    }

    public ReliableWriterCacheEventCountSeq (Collection elements) {
        super(ReliableWriterCacheEventCount.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ReliableWriterCacheEventCount.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ReliableWriterCacheEventCount objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ReliableWriterCacheEventCount.create());
