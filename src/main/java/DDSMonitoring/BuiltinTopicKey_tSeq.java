...
/**
* A sequence of BuiltinTopicKey_t instances.
*/
public final class BuiltinTopicKey_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BuiltinTopicKey_tSeq() {
        super(BuiltinTopicKey_t.class);
    }

    public BuiltinTopicKey_tSeq (int initialMaximum) {
        super(BuiltinTopicKey_t.class, initialMaximum);
    }

    public BuiltinTopicKey_tSeq (Collection elements) {
        super(BuiltinTopicKey_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BuiltinTopicKey_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BuiltinTopicKey_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BuiltinTopicKey_t.create());
