...
/**
* A sequence of TypeSupportQosPolicy instances.
*/
public final class TypeSupportQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TypeSupportQosPolicySeq() {
        super(TypeSupportQosPolicy.class);
    }

    public TypeSupportQosPolicySeq (int initialMaximum) {
        super(TypeSupportQosPolicy.class, initialMaximum);
    }

    public TypeSupportQosPolicySeq (Collection elements) {
        super(TypeSupportQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TypeSupportQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TypeSupportQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TypeSupportQosPolicy.create());
