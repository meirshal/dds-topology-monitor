...
/**
* A sequence of ReliableReaderActivityChangedStatus instances.
*/
public final class ReliableReaderActivityChangedStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ReliableReaderActivityChangedStatusSeq() {
        super(ReliableReaderActivityChangedStatus.class);
    }

    public ReliableReaderActivityChangedStatusSeq (int initialMaximum) {
        super(ReliableReaderActivityChangedStatus.class, initialMaximum);
    }

    public ReliableReaderActivityChangedStatusSeq (Collection elements) {
        super(ReliableReaderActivityChangedStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ReliableReaderActivityChangedStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ReliableReaderActivityChangedStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ReliableReaderActivityChangedStatus.create());
