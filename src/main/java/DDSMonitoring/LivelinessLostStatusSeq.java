...
/**
* A sequence of LivelinessLostStatus instances.
*/
public final class LivelinessLostStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LivelinessLostStatusSeq() {
        super(LivelinessLostStatus.class);
    }

    public LivelinessLostStatusSeq (int initialMaximum) {
        super(LivelinessLostStatus.class, initialMaximum);
    }

    public LivelinessLostStatusSeq (Collection elements) {
        super(LivelinessLostStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LivelinessLostStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LivelinessLostStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LivelinessLostStatus.create());
