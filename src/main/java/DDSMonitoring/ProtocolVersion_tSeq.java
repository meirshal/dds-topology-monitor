...
/**
* A sequence of ProtocolVersion_t instances.
*/
public final class ProtocolVersion_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ProtocolVersion_tSeq() {
        super(ProtocolVersion_t.class);
    }

    public ProtocolVersion_tSeq (int initialMaximum) {
        super(ProtocolVersion_t.class, initialMaximum);
    }

    public ProtocolVersion_tSeq (Collection elements) {
        super(ProtocolVersion_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ProtocolVersion_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ProtocolVersion_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ProtocolVersion_t.create());
