...
/**
* A sequence of RequestedIncompatibleQosStatus instances.
*/
public final class RequestedIncompatibleQosStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RequestedIncompatibleQosStatusSeq() {
        super(RequestedIncompatibleQosStatus.class);
    }

    public RequestedIncompatibleQosStatusSeq (int initialMaximum) {
        super(RequestedIncompatibleQosStatus.class, initialMaximum);
    }

    public RequestedIncompatibleQosStatusSeq (Collection elements) {
        super(RequestedIncompatibleQosStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RequestedIncompatibleQosStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RequestedIncompatibleQosStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RequestedIncompatibleQosStatus.create());
