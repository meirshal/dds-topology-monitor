...
/**
* A sequence of TransportMulticastMappingQosPolicy instances.
*/
public final class TransportMulticastMappingQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportMulticastMappingQosPolicySeq() {
        super(TransportMulticastMappingQosPolicy.class);
    }

    public TransportMulticastMappingQosPolicySeq (int initialMaximum) {
        super(TransportMulticastMappingQosPolicy.class, initialMaximum);
    }

    public TransportMulticastMappingQosPolicySeq (Collection elements) {
        super(TransportMulticastMappingQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportMulticastMappingQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportMulticastMappingQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportMulticastMappingQosPolicy.create());
