...
/**
* A sequence of PublishModeQosPolicy instances.
*/
public final class PublishModeQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PublishModeQosPolicySeq() {
        super(PublishModeQosPolicy.class);
    }

    public PublishModeQosPolicySeq (int initialMaximum) {
        super(PublishModeQosPolicy.class, initialMaximum);
    }

    public PublishModeQosPolicySeq (Collection elements) {
        super(PublishModeQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PublishModeQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PublishModeQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PublishModeQosPolicy.create());
