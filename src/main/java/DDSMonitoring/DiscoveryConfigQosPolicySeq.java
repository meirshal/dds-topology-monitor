...
/**
* A sequence of DiscoveryConfigQosPolicy instances.
*/
public final class DiscoveryConfigQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DiscoveryConfigQosPolicySeq() {
        super(DiscoveryConfigQosPolicy.class);
    }

    public DiscoveryConfigQosPolicySeq (int initialMaximum) {
        super(DiscoveryConfigQosPolicy.class, initialMaximum);
    }

    public DiscoveryConfigQosPolicySeq (Collection elements) {
        super(DiscoveryConfigQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DiscoveryConfigQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DiscoveryConfigQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DiscoveryConfigQosPolicy.create());
