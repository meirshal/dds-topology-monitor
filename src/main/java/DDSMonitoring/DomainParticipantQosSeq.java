...
/**
* A sequence of DomainParticipantQos instances.
*/
public final class DomainParticipantQosSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DomainParticipantQosSeq() {
        super(DomainParticipantQos.class);
    }

    public DomainParticipantQosSeq (int initialMaximum) {
        super(DomainParticipantQos.class, initialMaximum);
    }

    public DomainParticipantQosSeq (Collection elements) {
        super(DomainParticipantQos.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DomainParticipantQos.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DomainParticipantQos objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DomainParticipantQos.create());
