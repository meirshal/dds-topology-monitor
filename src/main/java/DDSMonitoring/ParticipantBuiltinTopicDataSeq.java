...
/**
* A sequence of ParticipantBuiltinTopicData instances.
*/
public final class ParticipantBuiltinTopicDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ParticipantBuiltinTopicDataSeq() {
        super(ParticipantBuiltinTopicData.class);
    }

    public ParticipantBuiltinTopicDataSeq (int initialMaximum) {
        super(ParticipantBuiltinTopicData.class, initialMaximum);
    }

    public ParticipantBuiltinTopicDataSeq (Collection elements) {
        super(ParticipantBuiltinTopicData.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ParticipantBuiltinTopicData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ParticipantBuiltinTopicData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ParticipantBuiltinTopicData.create());
