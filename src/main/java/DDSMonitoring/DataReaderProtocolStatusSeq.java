...
/**
* A sequence of DataReaderProtocolStatus instances.
*/
public final class DataReaderProtocolStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DataReaderProtocolStatusSeq() {
        super(DataReaderProtocolStatus.class);
    }

    public DataReaderProtocolStatusSeq (int initialMaximum) {
        super(DataReaderProtocolStatus.class, initialMaximum);
    }

    public DataReaderProtocolStatusSeq (Collection elements) {
        super(DataReaderProtocolStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DataReaderProtocolStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DataReaderProtocolStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DataReaderProtocolStatus.create());
