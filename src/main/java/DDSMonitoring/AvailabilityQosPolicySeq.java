...
/**
* A sequence of AvailabilityQosPolicy instances.
*/
public final class AvailabilityQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public AvailabilityQosPolicySeq() {
        super(AvailabilityQosPolicy.class);
    }

    public AvailabilityQosPolicySeq (int initialMaximum) {
        super(AvailabilityQosPolicy.class, initialMaximum);
    }

    public AvailabilityQosPolicySeq (Collection elements) {
        super(AvailabilityQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, AvailabilityQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' AvailabilityQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(AvailabilityQosPolicy.create());
