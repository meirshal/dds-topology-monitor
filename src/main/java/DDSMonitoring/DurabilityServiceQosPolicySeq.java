...
/**
* A sequence of DurabilityServiceQosPolicy instances.
*/
public final class DurabilityServiceQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DurabilityServiceQosPolicySeq() {
        super(DurabilityServiceQosPolicy.class);
    }

    public DurabilityServiceQosPolicySeq (int initialMaximum) {
        super(DurabilityServiceQosPolicy.class, initialMaximum);
    }

    public DurabilityServiceQosPolicySeq (Collection elements) {
        super(DurabilityServiceQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DurabilityServiceQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DurabilityServiceQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DurabilityServiceQosPolicy.create());
