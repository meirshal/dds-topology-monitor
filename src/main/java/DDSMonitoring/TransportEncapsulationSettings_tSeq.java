...
/**
* A sequence of TransportEncapsulationSettings_t instances.
*/
public final class TransportEncapsulationSettings_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportEncapsulationSettings_tSeq() {
        super(TransportEncapsulationSettings_t.class);
    }

    public TransportEncapsulationSettings_tSeq (int initialMaximum) {
        super(TransportEncapsulationSettings_t.class, initialMaximum);
    }

    public TransportEncapsulationSettings_tSeq (Collection elements) {
        super(TransportEncapsulationSettings_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportEncapsulationSettings_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportEncapsulationSettings_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportEncapsulationSettings_t.create());
