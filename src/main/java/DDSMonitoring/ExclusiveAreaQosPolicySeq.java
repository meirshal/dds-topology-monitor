...
/**
* A sequence of ExclusiveAreaQosPolicy instances.
*/
public final class ExclusiveAreaQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ExclusiveAreaQosPolicySeq() {
        super(ExclusiveAreaQosPolicy.class);
    }

    public ExclusiveAreaQosPolicySeq (int initialMaximum) {
        super(ExclusiveAreaQosPolicy.class, initialMaximum);
    }

    public ExclusiveAreaQosPolicySeq (Collection elements) {
        super(ExclusiveAreaQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ExclusiveAreaQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ExclusiveAreaQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ExclusiveAreaQosPolicy.create());
