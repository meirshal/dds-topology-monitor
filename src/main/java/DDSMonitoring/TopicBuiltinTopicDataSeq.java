...
/**
* A sequence of TopicBuiltinTopicData instances.
*/
public final class TopicBuiltinTopicDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TopicBuiltinTopicDataSeq() {
        super(TopicBuiltinTopicData.class);
    }

    public TopicBuiltinTopicDataSeq (int initialMaximum) {
        super(TopicBuiltinTopicData.class, initialMaximum);
    }

    public TopicBuiltinTopicDataSeq (Collection elements) {
        super(TopicBuiltinTopicData.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TopicBuiltinTopicData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TopicBuiltinTopicData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TopicBuiltinTopicData.create());
