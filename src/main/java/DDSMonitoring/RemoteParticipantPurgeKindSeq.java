...
/**
* A sequence of RemoteParticipantPurgeKind instances.
*/
public final class RemoteParticipantPurgeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RemoteParticipantPurgeKindSeq() {
        super(RemoteParticipantPurgeKind.class);
    }

    public RemoteParticipantPurgeKindSeq (int initialMaximum) {
        super(RemoteParticipantPurgeKind.class, initialMaximum);
    }

    public RemoteParticipantPurgeKindSeq (Collection elements) {
        super(RemoteParticipantPurgeKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RemoteParticipantPurgeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RemoteParticipantPurgeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RemoteParticipantPurgeKind.create());
