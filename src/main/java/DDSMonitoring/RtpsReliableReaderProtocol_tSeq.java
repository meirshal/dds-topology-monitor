...
/**
* A sequence of RtpsReliableReaderProtocol_t instances.
*/
public final class RtpsReliableReaderProtocol_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RtpsReliableReaderProtocol_tSeq() {
        super(RtpsReliableReaderProtocol_t.class);
    }

    public RtpsReliableReaderProtocol_tSeq (int initialMaximum) {
        super(RtpsReliableReaderProtocol_t.class, initialMaximum);
    }

    public RtpsReliableReaderProtocol_tSeq (Collection elements) {
        super(RtpsReliableReaderProtocol_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RtpsReliableReaderProtocol_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RtpsReliableReaderProtocol_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RtpsReliableReaderProtocol_t.create());
