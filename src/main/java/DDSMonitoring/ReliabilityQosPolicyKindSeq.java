...
/**
* A sequence of ReliabilityQosPolicyKind instances.
*/
public final class ReliabilityQosPolicyKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ReliabilityQosPolicyKindSeq() {
        super(ReliabilityQosPolicyKind.class);
    }

    public ReliabilityQosPolicyKindSeq (int initialMaximum) {
        super(ReliabilityQosPolicyKind.class, initialMaximum);
    }

    public ReliabilityQosPolicyKindSeq (Collection elements) {
        super(ReliabilityQosPolicyKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ReliabilityQosPolicyKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ReliabilityQosPolicyKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ReliabilityQosPolicyKind.create());
