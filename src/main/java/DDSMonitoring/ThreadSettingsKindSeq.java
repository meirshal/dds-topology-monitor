...
/**
* A sequence of ThreadSettingsKind instances.
*/
public final class ThreadSettingsKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ThreadSettingsKindSeq() {
        super(ThreadSettingsKind.class);
    }

    public ThreadSettingsKindSeq (int initialMaximum) {
        super(ThreadSettingsKind.class, initialMaximum);
    }

    public ThreadSettingsKindSeq (Collection elements) {
        super(ThreadSettingsKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ThreadSettingsKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ThreadSettingsKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ThreadSettingsKind.create());
