...
/**
* A sequence of UserObjectSettings_t instances.
*/
public final class UserObjectSettings_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public UserObjectSettings_tSeq() {
        super(UserObjectSettings_t.class);
    }

    public UserObjectSettings_tSeq (int initialMaximum) {
        super(UserObjectSettings_t.class, initialMaximum);
    }

    public UserObjectSettings_tSeq (Collection elements) {
        super(UserObjectSettings_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, UserObjectSettings_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' UserObjectSettings_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(UserObjectSettings_t.create());
