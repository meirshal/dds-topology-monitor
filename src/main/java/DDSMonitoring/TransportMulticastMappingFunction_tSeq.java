...
/**
* A sequence of TransportMulticastMappingFunction_t instances.
*/
public final class TransportMulticastMappingFunction_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportMulticastMappingFunction_tSeq() {
        super(TransportMulticastMappingFunction_t.class);
    }

    public TransportMulticastMappingFunction_tSeq (int initialMaximum) {
        super(TransportMulticastMappingFunction_t.class, initialMaximum);
    }

    public TransportMulticastMappingFunction_tSeq (Collection elements) {
        super(TransportMulticastMappingFunction_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportMulticastMappingFunction_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportMulticastMappingFunction_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportMulticastMappingFunction_t.create());
