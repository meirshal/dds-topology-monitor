...
/**
* A sequence of DiscoveryBuiltinReaderFragmentationResourceLimits_t instances.
*/
public final class DiscoveryBuiltinReaderFragmentationResourceLimits_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DiscoveryBuiltinReaderFragmentationResourceLimits_tSeq() {
        super(DiscoveryBuiltinReaderFragmentationResourceLimits_t.class);
    }

    public DiscoveryBuiltinReaderFragmentationResourceLimits_tSeq (int initialMaximum) {
        super(DiscoveryBuiltinReaderFragmentationResourceLimits_t.class, initialMaximum);
    }

    public DiscoveryBuiltinReaderFragmentationResourceLimits_tSeq (Collection elements) {
        super(DiscoveryBuiltinReaderFragmentationResourceLimits_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DiscoveryBuiltinReaderFragmentationResourceLimits_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DiscoveryBuiltinReaderFragmentationResourceLimits_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DiscoveryBuiltinReaderFragmentationResourceLimits_t.create());
