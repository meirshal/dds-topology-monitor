...
/**
* A sequence of SubscriptionBuiltinTopicData instances.
*/
public final class SubscriptionBuiltinTopicDataSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SubscriptionBuiltinTopicDataSeq() {
        super(SubscriptionBuiltinTopicData.class);
    }

    public SubscriptionBuiltinTopicDataSeq (int initialMaximum) {
        super(SubscriptionBuiltinTopicData.class, initialMaximum);
    }

    public SubscriptionBuiltinTopicDataSeq (Collection elements) {
        super(SubscriptionBuiltinTopicData.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SubscriptionBuiltinTopicData.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SubscriptionBuiltinTopicData objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SubscriptionBuiltinTopicData.create());
