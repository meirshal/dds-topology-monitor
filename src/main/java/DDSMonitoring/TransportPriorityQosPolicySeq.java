...
/**
* A sequence of TransportPriorityQosPolicy instances.
*/
public final class TransportPriorityQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportPriorityQosPolicySeq() {
        super(TransportPriorityQosPolicy.class);
    }

    public TransportPriorityQosPolicySeq (int initialMaximum) {
        super(TransportPriorityQosPolicy.class, initialMaximum);
    }

    public TransportPriorityQosPolicySeq (Collection elements) {
        super(TransportPriorityQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportPriorityQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportPriorityQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportPriorityQosPolicy.create());
