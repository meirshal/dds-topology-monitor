...
/**
* A sequence of RTIInstanceHandle instances.
*/
public final class RTIInstanceHandleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RTIInstanceHandleSeq() {
        super(RTIInstanceHandle.class);
    }

    public RTIInstanceHandleSeq (int initialMaximum) {
        super(RTIInstanceHandle.class, initialMaximum);
    }

    public RTIInstanceHandleSeq (Collection elements) {
        super(RTIInstanceHandle.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RTIInstanceHandle.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RTIInstanceHandle objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RTIInstanceHandle.create());
