...
/**
* A sequence of SubscriberProtocolQosPolicy instances.
*/
public final class SubscriberProtocolQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SubscriberProtocolQosPolicySeq() {
        super(SubscriberProtocolQosPolicy.class);
    }

    public SubscriberProtocolQosPolicySeq (int initialMaximum) {
        super(SubscriberProtocolQosPolicy.class, initialMaximum);
    }

    public SubscriberProtocolQosPolicySeq (Collection elements) {
        super(SubscriberProtocolQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SubscriberProtocolQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SubscriberProtocolQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SubscriberProtocolQosPolicy.create());
