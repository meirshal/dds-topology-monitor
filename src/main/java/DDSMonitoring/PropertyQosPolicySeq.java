...
/**
* A sequence of PropertyQosPolicy instances.
*/
public final class PropertyQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PropertyQosPolicySeq() {
        super(PropertyQosPolicy.class);
    }

    public PropertyQosPolicySeq (int initialMaximum) {
        super(PropertyQosPolicy.class, initialMaximum);
    }

    public PropertyQosPolicySeq (Collection elements) {
        super(PropertyQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PropertyQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PropertyQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PropertyQosPolicy.create());
