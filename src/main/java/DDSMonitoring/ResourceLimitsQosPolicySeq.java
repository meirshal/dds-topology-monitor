...
/**
* A sequence of ResourceLimitsQosPolicy instances.
*/
public final class ResourceLimitsQosPolicySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ResourceLimitsQosPolicySeq() {
        super(ResourceLimitsQosPolicy.class);
    }

    public ResourceLimitsQosPolicySeq (int initialMaximum) {
        super(ResourceLimitsQosPolicy.class, initialMaximum);
    }

    public ResourceLimitsQosPolicySeq (Collection elements) {
        super(ResourceLimitsQosPolicy.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ResourceLimitsQosPolicy.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ResourceLimitsQosPolicy objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ResourceLimitsQosPolicy.create());
