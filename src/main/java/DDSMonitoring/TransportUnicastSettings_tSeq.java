...
/**
* A sequence of TransportUnicastSettings_t instances.
*/
public final class TransportUnicastSettings_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransportUnicastSettings_tSeq() {
        super(TransportUnicastSettings_t.class);
    }

    public TransportUnicastSettings_tSeq (int initialMaximum) {
        super(TransportUnicastSettings_t.class, initialMaximum);
    }

    public TransportUnicastSettings_tSeq (Collection elements) {
        super(TransportUnicastSettings_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransportUnicastSettings_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransportUnicastSettings_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransportUnicastSettings_t.create());
