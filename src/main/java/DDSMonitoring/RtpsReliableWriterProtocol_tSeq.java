...
/**
* A sequence of RtpsReliableWriterProtocol_t instances.
*/
public final class RtpsReliableWriterProtocol_tSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RtpsReliableWriterProtocol_tSeq() {
        super(RtpsReliableWriterProtocol_t.class);
    }

    public RtpsReliableWriterProtocol_tSeq (int initialMaximum) {
        super(RtpsReliableWriterProtocol_t.class, initialMaximum);
    }

    public RtpsReliableWriterProtocol_tSeq (Collection elements) {
        super(RtpsReliableWriterProtocol_t.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RtpsReliableWriterProtocol_t.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RtpsReliableWriterProtocol_t objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RtpsReliableWriterProtocol_t.create());
