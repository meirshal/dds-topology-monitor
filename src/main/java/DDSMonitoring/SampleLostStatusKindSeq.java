...
/**
* A sequence of SampleLostStatusKind instances.
*/
public final class SampleLostStatusKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SampleLostStatusKindSeq() {
        super(SampleLostStatusKind.class);
    }

    public SampleLostStatusKindSeq (int initialMaximum) {
        super(SampleLostStatusKind.class, initialMaximum);
    }

    public SampleLostStatusKindSeq (Collection elements) {
        super(SampleLostStatusKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SampleLostStatusKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SampleLostStatusKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SampleLostStatusKind.create());
